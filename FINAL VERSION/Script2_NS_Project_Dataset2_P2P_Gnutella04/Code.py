...
data = open('p2p-Gnutella04.txt').readlines()
G = nx.DiGraph()
for row in data:
    nodeStrip = row.split()
...
def query_processing_using_k_truss(G,k,q):
    ...
    print("Algorithm 2 is executed")
    return Communities 
communities_algo2 = query_processing_using_k_truss(G,4,3116)
algo2 = open("Algo2_Output.txt", "w")
algo2.write(str(communities_algo2))
algo2.close()

def TCP_Index(G):
    print("Algorithm 3 has started")
    weight={}
    T={}
    S={}
    for x in G.nodes():
                    ...
                    for V_lst in V.values():
                        for z in V_lst:
                            ...
                            if (z,x) not in visited:
                                Q.append((z,x))
            Communities.append(C1)
    print("Algorithm 4 is executed")
    return Communities
communities_algo4 = Query_Processing_TCP(G,4,3116)
